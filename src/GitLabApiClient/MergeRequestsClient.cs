...
namespace GitLabApiClient
{
    ...
    public sealed class MergeRequestsClient : IMergeRequestsClient
    {
        ...
        public async Task<IList<MergeRequest>> GetAsync(ProjectId projectId,
            Action<ProjectMergeRequestsQueryOptions> options = null)
        {
            var projectMergeRequestOptions = new ProjectMergeRequestsQueryOptions();
            options?.Invoke(projectMergeRequestOptions);

            string query = _projectMergeRequestsQueryBuilder.Build($"projects/{projectId}/merge_requests",
                projectMergeRequestOptions);
        ...
        public async Task<IList<MergeRequest>> GetAsync(Action<MergeRequestsQueryOptions> options = null)
        {
            ...
            string query = _mergeRequestsQueryBuilder.Build("merge_requests", mergeRequestOptions);
        ...
        {
            return await _httpFacade.Get<MergeRequest>($"projects/{projectId}/merge_requests/{mergeRequestId}");
        }
        ...
        {
            return await _httpFacade.Post<MergeRequest>($"projects/{projectId}/merge_requests", request);
        }
        ...
        public async Task<MergeRequest>
            UpdateAsync(ProjectId projectId, int mergeRequestId, UpdateMergeRequest request)
        {
            return await _httpFacade.Put<MergeRequest>($"projects/{projectId}/merge_requests/{mergeRequestId}",
                request);
        }
        ...
        public async Task<Note> CreateNoteAsync(ProjectId projectId, int mergeRequestId,
            CreateMergeRequestNoteRequest request)
        {
            return await _httpFacade.Post<Note>($"projects/{projectId}/merge_requests/{mergeRequestId}/notes", request);
        }
        ...
        /// <returns>Merge requests satisfying options.</returns>
        public async Task<IList<Note>> GetNotesAsync(ProjectId projectId, int mergeRequestIid,
            Action<MergeRequestNotesQueryOptions> options = null)
        {
            ...
            string url =
                _projectMergeRequestNotesQueryBuilder.Build(
                    $"projects/{projectId}/merge_requests/{mergeRequestIid}/notes", queryOptions);
        ...
        {
            return await _httpFacade.Get<List<Pipeline>>(
                $"projects/{projectId}/merge_requests/{mergeRequestId}/pipelines");
        }
        ...
        {
            return await _httpFacade.GetPagedList<Discussion>(
        ...
        {
            return await _httpFacade.GetPagedList<AwardEmoji>(
