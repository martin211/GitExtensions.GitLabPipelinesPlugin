...
namespace GitLabApiClient.Internal.Http
{
    internal sealed class GitLabHttpFacade
    {
        ...
        {
            _httpClient =
                new HttpClient(httpMessageHandler ?? new HttpClientHandler()) { BaseAddress = new Uri(hostUrl) };
        ...
        public GitLabHttpFacade(string hostUrl, RequestsJsonSerializer jsonSerializer, string authenticationToken = "",
            HttpMessageHandler httpMessageHandler = null, TimeSpan? clientTimeout = null) : this(hostUrl, jsonSerializer, httpMessageHandler, clientTimeout)
        {
        ...
        {
            return _pagedRequestor.GetPagedList<T>(uri);
        }

        public Task<T> Get<T>(string uri)
        {
            return _requestor.Get<T>(uri);
        }

        public Task GetFile(string uri, string outputPath)
        {
            return _requestor.GetFile(uri, outputPath);
        }

        public Task<T> Post<T>(string uri, object data = null) where T : class
        {
            return _requestor.Post<T>(uri, data);
        }

        public Task Post(string uri, object data = null)
        {
            return _requestor.Post(uri, data);
        }

        public Task<Upload> PostFile(string uri, CreateUploadRequest uploadRequest)
        {
            return _requestor.PostFile(uri, uploadRequest);
        }

        public Task<T> PostFile<T>(string uri, Dictionary<string, string> keyValues,
            CreateUploadRequest uploadRequest)
        {
            return _requestor.PostFile<T>(uri, keyValues, uploadRequest);
        }

        public Task<T> Put<T>(string uri, object data)
        {
            return _requestor.Put<T>(uri, data);
        }

        public Task Put(string uri, object data)
        {
            return _requestor.Put(uri, data);
        }

        public Task Delete(string uri)
        {
            return _requestor.Delete(uri);
        }

        public Task Delete(string uri, object data)
        {
            return _requestor.Delete(uri, data);
        }
